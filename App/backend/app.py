...
CORS(app, resources={r"/*": {"origins": cors_origins}}, supports_credentials=True)  # Allow all origins for simplicity; adjust as needed
...
def download_img():
    ...
    if province is not None or district is not None or ward is not None:
        ...
        logger.info('debug 4')
        if "lst_img" not in data or not data["lst_img"]:
            if not geo_series.any():
                return jsonify({'message': 'No images found for the provided location', 'status': 400})
            save_npy(geo_series, G, data)
        logger.info('debug 5')
        for idx, bound in enumerate(geo_series):
            print(f'Image {idx+1} of {len(geo_series)}')
            try:
    ...
    try:
        ...
        if province is not None or district is not None or ward is not None:
            ...
            if isinstance(mask, np.ndarray) and isinstance(big_images, np.ndarray):
                ...
                file_path = os.path.join('/frontend/static', 'img', filename)
